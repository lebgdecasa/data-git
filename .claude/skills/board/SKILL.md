...
1. **Intel sweep (silent):** read `foundry/vault/personal/profile.md`, skim `foundry/war-room/decisions.md` for relevant prior decisions, and pull whatever `foundry/vault/` files bear on the question (offers/pricing/leads → `vault/hormozi/doctrine.md`).
...
6. **Log:** if the session produced a real decision, append it to `foundry/war-room/decisions.md` (date | decision | rationale | deadline | status: active). Mention you logged it. Don't log exploratory chatter.
