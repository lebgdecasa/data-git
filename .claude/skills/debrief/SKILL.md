---
name: debrief
description: End-of-session ritual — log decisions and artifacts to the war room, update profile/pipeline if intel changed, commit and push so nothing is lost when the session dies. Usage - /debrief [optional notes].
---

Close out the current session. No cinematic opening — this is the quiet after the war room empties.
...
2. **Write the debrief** to `foundry/war-room/sessions/<YYYY-MM-DD>-<topic-slug>.md`:
   - TL;DR (3 lines max)
...
3. **Update the ledgers:**
   - Append decisions to `foundry/war-room/decisions.md` (skip duplicates already logged mid-session).
...
4. **Persist:** `git add` the foundry files, commit with message `war room: <date> <topic>` and push to the current branch (retry per repo conventions). Sessions are ephemeral; the repo is the brain.
